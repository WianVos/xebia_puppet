		...
		case $opendj::install {
			source : {
				...
				exec {
					"opendj install" :
						command =>
						"${opendj::homedir}/setup --cli --baseDN ${basedn} --ldapPort ${opendj::ldapport} --adminConnectorPort ${opendj::mgtport} --rootUserDN \'${opendj::rootuser}\' --rootUserPassword ${opendj::rootpassword} --no-prompt --noPropertiesFile && touch ${opendj::homedir}/puppetinstalledopendj.txt ",
